...
#include "../cas/cas_ioctl.h"

#include "cas_control.h"
...
tArgs vArgs[] =
{
	{ "-c", " --setCam  ", "Args: No argumens\n\tSet cam mode" },
	{ "-m", " --setMm  ", "Args: No argumens\n\tSet mm mode" },
...
void usage(char *prg, char *cmd)
{
	...
	FILE *fcas = fopen(CAS_DEVICE, "r");

	/* or printout a default usage */
	fprintf(stderr, "Cas Programmer control tool, version 1.00\n");
	if (fcas)
	{
		fd = open(CAS_DEVICE, O_RDWR);
		if (fd < 0)
		{
		...
		char device_name[64];
		if (ioctl(fd, IOCTL_DEVICE_INFORMATION_COMMAND, &cas_info_cmd) < 0)
		{
			fprintf(stderr, "Failed send ioctl command: IOCTL_DEVICE_INFORMATION_COMMAND, (%m)\n");
			exit(1);
		}
		fprintf(stderr, "Found device: %s, vid: 0x%04x, pid: 0x%04x, status: %s \n", cas_device_list[cas_info_cmd.device], cas_info_cmd.vid, cas_info_cmd.pid, cas_device_status[cas_info_cmd.status]);
	}
	else
	{
...
int main(int argc, char *argv[])
{
	...
	if (argc > 1)
	{
		...
		while (i < argc)
		{
			if ((strcmp(argv[i], "-c") == 0) || (strcmp(argv[i], "--setCam") == 0))
			{
				fd = open(CAS_DEVICE, O_RDWR);
				if (fd < 0)
				{
				...
				if (ioctl(fd, IOCTL_SET_CAM) < 0)
				{
					fprintf(stderr, "Failed send ioctl command: IOCTL_SET_CAM, (%m)\n");
					exit(1);
				}
			}
			else if ((strcmp(argv[i], "-m") == 0) || (strcmp(argv[i], "--setMm") == 0))
			{
				fd = open(CAS_DEVICE, O_RDWR);
				if (fd < 0)
				{
				...
				if (ioctl(fd, IOCTL_SET_MM) < 0)
				{
					fprintf(stderr, "Failed send ioctl command: IOCTL_SET_MM, (%m)\n");
					exit(1);
				}
			}
			else if ((strcmp(argv[i], "-m") == 0) || (strcmp(argv[i], "--setMm") == 0))
			{
				...
				if (ioctl(fd, IOCTL_SET_JTAG) < 0)
				{
					fprintf(stderr, "Failed send ioctl command: IOCTL_SET_JTAG, (%m)\n");
					exit(1);
				}
			}
			...
			else if ((strcmp(argv[i], "-h") == 0) || (strcmp(argv[i], "--setHost") == 0))
			{
				fd = open(CAS_DEVICE, O_RDWR);
				if (fd < 0)
				{
				...
				if (ioctl(fd, IOCTL_SET_HOST) < 0)
				{
					fprintf(stderr, "Failed send ioctl command: IOCTL_SET_HOST, (%m)\n");
					exit(1);
				}
			}
			else
